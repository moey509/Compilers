...
expr returns [CubexExpression cub]
  ...
  | name=(VARFUN | CLASSID) LANGLE tes=exprs RANGLE LPAREN pes=exprs RPAREN { $cub = new CubexFunctionCall($name.text, $tes.cub, $pes.cub) }
  | name=(VARFUN | CLASSID) LPAREN pes=exprs RPAREN { $cub = new CubexFunctionCall($name.text, $pes.cub) }
...
exprs returns [List<CubexExpression> cub] 
  : { $cub = new ArrayList<CubexExpression>(); }
...
iface
  : INTERFACE name=(VARFUN | CLASSID) LANGLE kcont RANGLE (EXTENDS type)? LBRACE func* RBRACE
;

classgrammar
  : CLASS (VARFUN | CLASSID) (LANGLE kcont RANGLE)? LPAREN tcont RPAREN (EXTENDS type)? LBRACE statement* (SUPER LPAREN expr* RPAREN SEMICOLON)? func* RBRACE
;
